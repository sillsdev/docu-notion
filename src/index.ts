...
let docsOutputPath = "not set yet";
...
export async function notionPull(): Promise<void> {
  ...
  docsOutputPath = process.argv[3] || "./docs";
  imageOutputPath = process.argv[4] || "/static/notion_img";

  console.log(`notion_token:${token}`);
  console.log(`docsOutputPath:${docsOutputPath}`);
  console.log(`imageOutputPath:${imageOutputPath}`);
  ...
  deleteDirectorySync(docsOutputPath);
