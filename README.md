...
| --image-file-name-format &lt;format&gt;     |           | choices:<ul><li>`default`: {page slug (if any)}.{original file name (if it can be determined)}.{image block ID}</li><li>`content-hash`: Use a hash of the image content.</li><li>`legacy`: Use the legacy (&lt;= v0.15) method of determining file names. Set this to maintain backward compatibility.</li></ul>All formats will use the original file extension. |
